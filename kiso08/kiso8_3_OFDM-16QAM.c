...
///////////////////////////////////BER特性算出時に用いる変数　　/////////////////////////////////////////////////
	...
	printf("%dパス準静的レイリーフェージング(16QAM)，%ddB減衰モデル　s=%d\n",L,dB,s);
	printf(" ρ[dB]         BER \n");
	for(EbN0=0; EbN0<=35; EbN0+=5){
		...
		for(m=0; m<frame; m++){	
			...
			///  マルチパスフェージングのインパルス応答は時変＝フレームごとに違うものを作る
			...
			//1フレーム1ガウス雑音なので，h(t)の電力は所望のP_multifd[i]にならないが，frameでループすることによって，最終的に平均電力は希望通りになる
			}
			...
			for(j=0;j<K;j++){
				///////////////////////////  I軸判定　　//////////////////////////////////////////
				////////////    (00)伝送時の判定   /////////////
				if(creal(sF2[j*s])>=2*amp){
					if(0<=creal(rF[j*s]) && creal(rF[j*s])<2*amp){	
						ci++;	//(00)伝送時(01)と誤る
					}else if(-2*amp<=creal(rF[j*s]) && creal(rF[j*s])<0){
						ci+=2;	//(00)伝送時(11)と誤る
					}else if(creal(rF[j*s])<=-2*amp){
						ci++;	//(00)伝送時(10)と誤る
					}
				}
					
				////////////    (01)伝送時の判定   /////////////
				if(0<=creal(sF2[j*s]) && creal(sF2[j*s])<2*amp){
					if(2*amp<=creal(rF[j*s])){	
						ci++;	//(01)伝送時(00)と誤る
					}else if(-2*amp<=creal(rF[j*s]) && creal(rF[j*s])<0){
						ci++;	//(01)伝送時(11)と誤る
					}else if(creal(rF[j*s])<=-2*amp){
				...
				////////////    (11)伝送時の判定   /////////////
				if(-2*amp<=creal(sF2[j*s]) && creal(sF2[j*s])<0){
					if(2*amp<=creal(rF[j*s])){	
						ci+=2;	//(11)伝送時(00)と誤る
					}else if(0<=creal(rF[j*s]) && creal(rF[j*s])<2*amp){
						ci++;	//(11)伝送時(01)と誤る
					}else if(creal(rF[j*s])<=-2*amp){
						ci++;	//(11)伝送時(10)と誤る
					}	
				...
				///////////////////////////  Q軸判定　　//////////////////////////////////////////
				////////////    (00)伝送時の判定   /////////////
				if(cimag(sF2[j*s])>=2*amp){
					if(0<=cimag(rF[j*s]) && cimag(rF[j*s])<2*amp){	
						cq++;	//(00)伝送時(01)と誤る
					}else if(-2*amp<=cimag(rF[j*s]) && cimag(rF[j*s])<0){
						cq+=2;	//(00)伝送時(11)と誤る
					}else if(cimag(rF[j*s])<=-2*amp){
						cq++;	//(00)伝送時(10)と誤る
					}
				}
					
				////////////    (01)伝送時の判定   /////////////
				if(0<=cimag(sF2[j*s]) && cimag(sF2[j*s])<2*amp){
					if(2*amp<=cimag(rF[j*s])){	
						cq++;	//(01)伝送時(00)と誤る
					}else if(-2*amp<=cimag(rF[j*s]) && cimag(rF[j*s])<0){
						cq++;	//(01)伝送時(11)と誤る
					}else if(cimag(rF[j*s])<=-2*amp){
				...
				////////////    (11)伝送時の判定   /////////////
				if(-2*amp<=cimag(sF2[j*s]) && cimag(sF2[j*s])<0){
					if(2*amp<=cimag(rF[j*s])){	
						cq+=2;	//(11)伝送時(00)と誤る
					}else if(0<=cimag(rF[j*s]) && cimag(rF[j*s])<2*amp){
						cq++;	//(11)伝送時(01)と誤る
					}else if(cimag(rF[j*s])<=-2*amp){
						cq++;	//(11)伝送時(10)と誤る
					}	
	...
	for(i=0;i<L;i++)
		printf("P_multifd=%f\t h(t)/frame=%f\n",P_multifd[i],Psum2[i]/frame);
