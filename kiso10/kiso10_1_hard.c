/*/////////////////////  NSC符号，RSC符号の硬判定復号(ハミング距離を用いる)　　///////////////////////////////
・RSC符号の状態線図がNSC符号と全く違うことに注意
・RSC符号の最終状態が自分の作り方だと遅延素子のケツの値ではなく，ケツの値にケツの入力が加わって遷移した状態
・雑音電力σ^2の式
...
///////////////////////      乱数の種を設定     ///////////////
	...
	int pass_metA[2][N*r+r];		//時刻tにおける状態Aでのパスメトリック
	int pass_metB[2][N*r+r];		//時刻tにおける状態Bでのパスメトリック
	int pass_metC[2][N*r+r];		//時刻tにおける状態Cでのパスメトリック
	int pass_metD[2][N*r+r];		//時刻tにおける状態Dでのパスメトリック
	int key;						//0=A(00)，1=B(10)，2=C(01)，3=D(11)
	// int pass_metA=0,pass_metB=0,pass_metC=0,pass_metD=0;
...
///////////////////////////////////BER特性算出時に用いる変数　　/////////////////////////////////////////////////
	...
	printf("符号長(符号語(テールビッド含めて))=%d, 符号化率=%f, 遅延素子数=%d, s=%d\n",r*(N*bit+delay_units),code_rate,delay_units,s);
	printf(" ρ[dB]         BER \n");
	for(EbN0=minEbN0; EbN0<=maxEbN0; EbN0++){
		ci=0.0; cq=0.0; 
		LOOP=first;			//Eb/N0の値ごとにfirst回でBERを軽く算出する
		//☆☆☆☆☆☆☆☆☆雑音の大きさ
		sigma=(double)BnTs/(2.0*s*bit)*pow(10.0,(-0.1*EbN0))*((2.0*N*bit+2.0*delay_units)/(N*bit));		//ガウス雑音の分散算出，
		for(loop=0; loop<LOOP; loop++){	
			...
			for(i=0; i<N*r+r; i++){		//☆☆☆雑音のくわえ方＿テールビットにも雑音くわえる？
					...
					// rt_QPSK[i]=codeword_QPSK[i];	//AWGNなし
			}
			...
			if(choise_code==1){
				...
				for(i=0; i<N*r+r; i++){
					if(i==0){
						for(j=0; j<2; j++){
							pass_metA[j][i]=(rt_bit[bit*i]+0)%2+(rt_bit[bit*i+1]+0)%2;	//A→A(00)とのハミング距離
							pass_metB[j][i]=(rt_bit[bit*i]+1)%2+(rt_bit[bit*i+1]+1)%2;	//A→B(11)とのハミング距離
						}
					}
					if(i==1){
						for(j=0; j<2; j++){
							pass_metA[j][i]=pass_metA[0][i-1]+(rt_bit[bit*i]+0)%2+(rt_bit[bit*i+1]+0)%2;	//A→A(00)とのハミング距離
							pass_metB[j][i]=pass_metA[0][i-1]+(rt_bit[bit*i]+1)%2+(rt_bit[bit*i+1]+1)%2;	//A→B(11)とのハミング距離
							pass_metC[j][i]=pass_metB[0][i-1]+(rt_bit[bit*i]+0)%2+(rt_bit[bit*i+1]+1)%2;	//B→C(01)とのハミング距離
							pass_metD[j][i]=pass_metB[0][i-1]+(rt_bit[bit*i]+1)%2+(rt_bit[bit*i+1]+0)%2;	//B→D(10)とのハミング距離
						}
					}
					if(i>=2){					
						pass_metA[0][i]=arg_min(pass_metA[0][i-1],pass_metA[1][i-1])+(rt_bit[bit*i]+0)%2+(rt_bit[bit*i+1]+0)%2;	//A→A passA+(00)とのハミング距離
						pass_metA[1][i]=arg_min(pass_metC[0][i-1],pass_metC[1][i-1])+(rt_bit[bit*i]+1)%2+(rt_bit[bit*i+1]+1)%2;	//C→A passC+(11)とのハミング距離
						pass_metB[0][i]=arg_min(pass_metA[0][i-1],pass_metA[1][i-1])+(rt_bit[bit*i]+1)%2+(rt_bit[bit*i+1]+1)%2;	//A→B passA+(11)とのハミング距離
						pass_metB[1][i]=arg_min(pass_metC[0][i-1],pass_metC[1][i-1])+(rt_bit[bit*i]+0)%2+(rt_bit[bit*i+1]+0)%2;	//C→B passC+(00)とのハミング距離
						pass_metC[0][i]=arg_min(pass_metB[0][i-1],pass_metB[1][i-1])+(rt_bit[bit*i]+0)%2+(rt_bit[bit*i+1]+1)%2;	//B→C passB+(01)とのハミング距離
						pass_metC[1][i]=arg_min(pass_metD[0][i-1],pass_metD[1][i-1])+(rt_bit[bit*i]+1)%2+(rt_bit[bit*i+1]+0)%2;	//D→C passD+(10)とのハミング距離
						pass_metD[0][i]=arg_min(pass_metB[0][i-1],pass_metB[1][i-1])+(rt_bit[bit*i]+1)%2+(rt_bit[bit*i+1]+0)%2;	//B→D passB+(10)とのハミング距離
						pass_metD[1][i]=arg_min(pass_metD[0][i-1],pass_metD[1][i-1])+(rt_bit[bit*i]+0)%2+(rt_bit[bit*i+1]+1)%2;	//D→D passD+(01)とのハミング距離		
					}
					
				}
				
				//硬判定ビタビ復号_ビタビアルゴリズム
				//ミス，例えばkey==1でBの判定に行きkey=2を得たらまたif分の中に入ってしまう
				/* 	key=0;	//最初は状態Aから判定?
				for(i=(N*bit+2)-1; i>=0; i--){
					if(key==0){		//状態A(00)での判定
						if(pass_metA[0][i]<=pass_metA[1][i])	key=0;	//次回状態Aで判定
						if(pass_metA[0][i]>pass_metA[1][i])		key=2;	//次回状態Cで判定
						estimate_tail[i]=0;		//状態Aでの判定は入力が必ず0
					}
					
					if(key==1){		//状態B(10)での判定
				...
				for(i=0; i<N*bit+2; i++){
					...
					if(i==1){
						for(j=0; j<2; j++){
							pass_metA[j][i]=pass_metA[0][i-1]+(rt_bit[bit*i]+0)%2+(rt_bit[bit*i+1]+0)%2;	//A→A(00)とのハミング距離
							pass_metB[j][i]=pass_metA[0][i-1]+(rt_bit[bit*i]+1)%2+(rt_bit[bit*i+1]+1)%2;	//A→B(11)とのハミング距離
							pass_metC[j][i]=pass_metB[0][i-1]+(rt_bit[bit*i]+1)%2+(rt_bit[bit*i+1]+0)%2;	//B→C(10)とのハミング距離
							pass_metD[j][i]=pass_metB[0][i-1]+(rt_bit[bit*i]+0)%2+(rt_bit[bit*i+1]+1)%2;	//B→D(01)とのハミング距離
						}
					}
					if(i>=2){					
						pass_metA[0][i]=arg_min(pass_metA[0][i-1],pass_metA[1][i-1])+(rt_bit[bit*i]+0)%2+(rt_bit[bit*i+1]+0)%2;	//A→A passA+(00)とのハミング距離
						...
						pass_metB[0][i]=arg_min(pass_metA[0][i-1],pass_metA[1][i-1])+(rt_bit[bit*i]+1)%2+(rt_bit[bit*i+1]+1)%2;	//A→B passA+(11)とのハミング距離
						pass_metB[1][i]=arg_min(pass_metC[0][i-1],pass_metC[1][i-1])+(rt_bit[bit*i]+0)%2+(rt_bit[bit*i+1]+0)%2;	//C→B passC+(00)とのハミング距離
						pass_metC[0][i]=arg_min(pass_metB[0][i-1],pass_metB[1][i-1])+(rt_bit[bit*i]+1)%2+(rt_bit[bit*i+1]+0)%2;	//B→C passB+(10)とのハミング距離
						pass_metC[1][i]=arg_min(pass_metD[0][i-1],pass_metD[1][i-1])+(rt_bit[bit*i]+0)%2+(rt_bit[bit*i+1]+1)%2;	//D→C passD+(01)とのハミング距離
						pass_metD[0][i]=arg_min(pass_metB[0][i-1],pass_metB[1][i-1])+(rt_bit[bit*i]+0)%2+(rt_bit[bit*i+1]+1)%2;	//B→D passB+(01)とのハミング距離
						pass_metD[1][i]=arg_min(pass_metD[0][i-1],pass_metD[1][i-1])+(rt_bit[bit*i]+1)%2+(rt_bit[bit*i+1]+0)%2;	//D→D passD+(10)とのハミング距離		
					}
			...
			// for(j=0;j<2; j++){
				// putchar('\n');
				// for(i=0;i<N*r+r;i++)
				// printf(" A:%d B:%d C:%d D:%d ",pass_metA[j][i],pass_metB[j][i],pass_metC[j][i],pass_metD[j][i]);
			// }
			...
			/// 誤り判定(先頭点のみ)  ////////////////////////////////////////////////////////////////////////
			for(j=0; j<K; j++){
				if(creal(st[j*s])>=0 && creal(estimate_QPSK[j*s])<0)	ci++; //printf("%d番目\n",j);}	
				if(creal(st[j*s])<0 && creal(estimate_QPSK[j*s])>=0) 	ci++;//printf("%d番目\n",j);}
				if(cimag(st[j*s])>=0 && cimag(estimate_QPSK[j*s])<0)  	cq++;	//printf("%d番目\n",j);}
				if(cimag(st[j*s])<0 && cimag(estimate_QPSK[j*s])>=0)  	cq++;	//printf("%d番目\n",j);}
			}	
			...
			//BERの値によってループ調整
			if(loop==LOOP-1){
				ber=(ci+cq)/(bit*(double)loop*K);		//一旦BERを算出
		...
		// putchar('\n');
		// printf("符号語 = ");
		// for(i=0;i<N*bit*r+2*r;i++)
			// printf("%d",codeword[i]);

		// putchar('\n');
		// for(i=0;i<N*r+r;i++)
			// printf("実部=%f，虚部=%f \n",creal(codeword_QPSK[i]),cimag(codeword_QPSK[i]));
		
		// putchar('\n');
		// for(i=0;i<N*r+r;i++)
			// printf("実部=%f，虚部=%f \n",creal(rt_QPSK[i]),cimag(rt_QPSK[i]));
		...
		// putchar('\n');
		// printf("推定ビット系列(テールビットあり)   =");
		// for(i=0;i<N*bit+2;i++)
			// printf("%d",estimate_tail[i]);
		// putchar('\n');
...
void RSC_state(int nd, int input[], int memory[]){
	...
	// if(D[0][nd-1]==0 && D[1][nd-1]==0)	memory[0]=0;	//最終状態はA
	// if(D[0][nd-1]==1 && D[1][nd-1]==0)	memory[0]=1;	//最終状態はB
	// if(D[0][nd-1]==0 && D[1][nd-1]==1)	memory[0]=2;	//最終状態はC
	// if(D[0][nd-1]==1 && D[1][nd-1]==1)	memory[0]=3;	//最終状態はD
