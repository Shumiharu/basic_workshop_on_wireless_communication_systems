/////////////////////  多重伝送 MIMO-OFDM-QPSK　ZF，MMSE，MLD等化　　///////////////////////////////
/*
・雑音電力の設定  σ^2=(Nt*Nt)/Nrが係数になる．→詳細は紙参照
...
#define Q 4					//変調多値数
#define choise 3			//1=ZF等化，2=MMSE等化，3=MLD
...
///////////////////////////////////BER特性算出時に用いる変数　　/////////////////////////////////////////////////
	...
	printf("%d×%dMIMO-OFDM(QPSK)%dパス準静的レイリーフェージング，%ddB減衰モデル　GI=%d\n",Nt,Nr,L,dB,GI);
	printf("ρ[dB]        BER \n");
	for(EbN0=0; EbN0<=50; EbN0+=5){
		...
		//↓↓↓↓↓↓↓↓正しい雑音電力の設定↓↓↓↓↓↓↓↓↓↓
		sigma=(Nt*Nt)/Nr*BnTs/(2*s*bit)*pow(10.0,(-0.1*EbN0))*((K+GI)/(double)K);		//奥村さん＆岡本先生が思う雑音電力σ^2
		...
		for(loop=0; loop<LOOP; loop++){	
			...
			for(m=0;m<Nr;m++){
				for(i=0;i<(K+GI)*s;i++){
					...
					// rt1[m][i]=st3[m][i];			//雑音を入れないver→BER=0となっているか確認用
				}
			}
			...
			if(choise==3)	MLD_QPSK(Hf_multifd,rF2,rhatF);
			...
			//BERの値によってループ調整
			if(loop==LOOP-1){
				ber=(ci+cq)/(bit*(double)(1+loop)*K*Nt);		//☆☆☆全部でNt個，一旦BERを算出
...
void boxmuller(int num,double complex noise[], double mean , double variance){
	...
	double (*nreal)=(double(*))malloc(num*sizeof(double));		//ガウス雑音実部
	double (*nimag)=(double(*))malloc(num*sizeof(double));		//ガウス雑音虚部
...
void inv_matrix(int num,int nd,double complex IN[num][num][nd],double complex OUT[num][num][nd]){
	...
	double complex (*temp)[num][nd]=(double complex(*)[num][nd])malloc(num*num*nd*sizeof(double complex));		
...
void ZF(double complex H[Nr][Nt][N],double complex r1[Nr][N],double complex r2[Nt][N]){
	...
	double complex (*H_inv)[Nt][N]=(double complex(*)[Nt][N])malloc(Nr*Nt*N*sizeof(double complex));		//Hの逆行列
	double complex (*tmp1)[1][N]=(double complex(*)[1][N])malloc(Nr*1*N*sizeof(double complex));		//tmp1(Nr×1)
	double complex (*tmp2)[1][N]=(double complex(*)[1][N])malloc(Nt*1*N*sizeof(double complex));		//tmp1(Nt×1)
	double complex (*H_transpose)[Nr][N]=(double complex(*)[Nr][N])malloc(Nt*Nr*N*sizeof(double complex));		//Hのエルミート転置
	double complex (*check)[Nt][N]=(double complex(*)[Nt][N])malloc(Nr*Nt*N*sizeof(double complex));		
	double complex (*W1)[Nt][N]=(double complex(*)[Nt][N])malloc(Nt*Nt*N*sizeof(double complex));		//重みW1(Nt×Nt)
	double complex (*W2)[Nt][N]=(double complex(*)[Nt][N])malloc(Nt*Nt*N*sizeof(double complex));		//重みW2(Nt×Nt)
	double complex (*W)[Nr][N]=(double complex(*)[Nr][N])malloc(Nt*Nr*N*sizeof(double complex));		//ZF等化の伝達関数W
	...
	free(H_inv);			free(tmp1);			free(tmp2);
	free(H_transpose);		free(check);		free(W1);
	free(W2);			free(W);		
...
void MMSE(double complex H[Nr][Nt][N],double variance,double complex r1[Nr][N],double complex r2[Nt][N]){
	...
	double complex (*tmp1)[1][N]=(double complex(*)[1][N])malloc(Nr*1*N*sizeof(double complex));		//tmp1(Nr×1)
	double complex (*tmp2)[1][N]=(double complex(*)[1][N])malloc(Nt*1*N*sizeof(double complex));		//tmp1(Nt×1)
	double complex (*H_transpose)[Nr][N]=(double complex(*)[Nr][N])malloc(Nt*Nr*N*sizeof(double complex));		//チャネル行列Hのエルミート転置H^H
	double complex (*W)[Nr][N]=(double complex(*)[Nr][N])malloc(Nt*Nr*N*sizeof(double complex));		//MMSEフィルタの伝達関数
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//MIMO基礎ワークショップ W=inv(H^H*H+σ^2*I_Nt)*H^H////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	double complex (*W1)[Nt][N]=(double complex(*)[Nt][N])malloc(Nt*Nt*N*sizeof(double complex));		//重みW1(Nt×Nt)
	double complex (*W2)[Nt][N]=(double complex(*)[Nt][N])malloc(Nt*Nt*N*sizeof(double complex));		//重みW2(Nt×Nt)
	...
	for(j=0;j<Nt;j++){	//単位行列I_Ntでないと行列の要素が合わない
		for(i=0;i<Nt;i++){
			for(k=0;k<N;k++){//σを足すのは全成分ではなく，単位行列をかけてるから，対角行列の場所だけ
			// W1[i][j][k]+=variance;			//間違い！→対角成分の満たす必要がある,varianceは受信アンテナ全体の雑音電力だからNrいらない	
			// W1[i][j][k]+=variance*Nr;		//指導書通りならこれ間違い，sigmaが受信アンテナ全体の雑音電力を表すから
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//メモリ開放
	free(tmp1);			free(tmp2);			free(H_transpose);
...
void MLD_QPSK(double complex H[Nr][Nt][N],double complex rt[Nr][N],double complex estimate[Nt][N]){
	...
	int (*perm_num)=(int(*))malloc(p*sizeof(int));
	...
	double complex (*replica)[q][N]=(double complex(*)[q][N])malloc(Nt*q*N*sizeof(double complex));		//送信信号の全組み合わせ，縦にNt個，横にQ^Nt個ある行列
	double complex (*Hx)[q][N]=(double complex(*)[q][N])malloc(Nr*q*N*sizeof(double complex));		//レプリカ(Nr×Q^Nt)
